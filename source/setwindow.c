	...
	while(1)
	{
		...
		if(re_show)
		{
			...
			DrawHZText12(msg,0,set_offset,y_offset+line_x*3,gl_color_selected,1);
				...
				Draw_select_icon(x_offset+12*3,y_offset+line_x*3,(language_sel == 2));
				sprintf(msg,"%s"," ENG");
				DrawHZText12(msg,0,x_offset+8,y_offset+line_x*3,(language_sel==0)?gl_color_selected:gl_color_text,1);
				sprintf(msg,"%s"," ESP");
				DrawHZText12(msg,0,x_offset+12*3+9,y_offset+line_x*3,(language_sel==2)?gl_color_selected:gl_color_text,1);
				sprintf(msg,"%s",gl_zh_lang);
				DrawHZText12(msg,0,x_offset+12*5+16,y_offset+line_x*3,(language_sel==1)?gl_color_selected:gl_color_text,1);

			//
			VBlankIntrWait();
			sprintf(msg,"%s",gl_engine);
			DrawHZText12(msg,0,set_offset,y_offset+line_x*4,gl_color_selected,1);
				Draw_select_icon(x_offset-10,y_offset+line_x*4,(engine_sel == 0x1));
				sprintf(msg,"%s",gl_use_engine);
				DrawHZText12(msg,0,x_offset+5,y_offset+line_x*4,(engine_pos==0)?gl_color_selected:gl_color_text,1);
			//
			...
			DrawHZText12(msg,0,set_offset,y_offset+line_x*7,gl_color_selected,1);
				Draw_select_icon(x_offset-32,y_offset+line_x*7,(gl_ingame_RTC_open_status == 0x1));
				sprintf(msg,"%s",gl_offRTC_powersave);
				DrawHZText12(msg,0,x_offset-18,y_offset+line_x*7,(RTC_pos==0)?gl_color_selected:gl_color_text,1);
			...
			case 1: //edit state
				//if(Set_OK_line==0) {
					...
					if(keysrepeat & KEY_UP) {
						...
						{
							if (language_sel == 0){ // 0 is English
								language_sel = 2; // 2 is Spanish
							} else if (language_sel == 2){
								language_sel = 1; // 1 is Chinese
							} else {
								language_sel = 1;
							}
						}
						...
						{
							if (language_sel == 1){ // 2 is English
								language_sel = 2; // 2 is Spanish
							} else if (language_sel == 2){
								language_sel = 0; // 0 is English
							} else {
								language_sel = 0;
							}
						}
...
{
	if(language_sel == 0){ //English
		SET_info_buffer[0] = 0xE1E1;
	}
	else if(language_sel == 1) // Chinese
	{
		SET_info_buffer[0] = 0xE2E2;
	}
	else // Spanish
	{
		SET_info_buffer[0] = 0xE3E3;
	}
